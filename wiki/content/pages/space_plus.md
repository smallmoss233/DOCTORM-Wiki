...
> 💡 “头部卡方块”被强制判定为无氧，是为了避免玩家在方块里被误判为“有氧”而不用消耗氧气。
...
> 这个算法让制氧机可以覆盖不规则形状的房间，但也意味着**墙壁**可能挡住洪泛。
...
- **AIT 的氧气逻辑被完全接管**——`Planet.hasOxygenInTank`、`OxygenatedEffect.isOxygenated` 这些方法都被重写成 DOCTOR M 的判定。
- **氧气量存在物品的 NBT 里**——每件宇航服 / 氧气罐都有自己的氧气值，可以不同。
- **屏幕 HUD 会检测“头部卡方块”**——这是为了避免玩家在方块里被判定为“有氧”而不用消耗氧气。
- **制氧机的多机合并**用弱引用缓存——世界卸载时缓存自动释放，不会内存泄漏。
- **水下制氧机比氧气补充机更好用**——前者只对周围玩家生效，后者需要手动右键，且冷却 32 秒。
- **喷气氧气罐的前摇**是为了让玩家看到“起飞前先聚能”，不然瞬间加速会显得突兀。
- **食用氧气罐的三个条件**（饥饿、力量、饥饿效果）其实是在致敬“人在绝境中会力大无穷”的梗。
